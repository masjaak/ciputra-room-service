...
import { CATEGORIES, MENU_ITEMS } from './App';
...
export default function App() {
        ...
        <div className="relative z-10 w-full max-w-[280px] mx-auto">
          <div className="flex justify-end mb-6">
            <div className="bg-black/30 backdrop-blur-md border border-white/20 px-3 py-1 rounded-full flex gap-3 text-[10px] font-medium text-white shadow-lg cursor-pointer hover:bg-black/40 transition-all">
              <span
                onClick={() => setLang('ID')}
                className={`${lang === 'ID' ? 'text-white font-bold' : 'text-white/50'}`}
              >
                ID
              </span>
              ...
              <span
                onClick={() => setLang('EN')}
                className={`${lang === 'EN' ? 'text-white font-bold' : 'text-white/50'}`}
              >
                EN
              </span>
            </div>
          ...
          <div className="text-center mb-8 animate-fade-in-up">
            <img
              ...
              alt="Logo" />
            <h1 className="text-3xl font-serif text-white drop-shadow-lg tracking-wide mb-2">
          ...
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 px-5 py-6 rounded-[1.5rem] shadow-2xl ring-1 ring-white/30 w-full">
            <div className="space-y-3">
              <div className="group">
                <label className="text-[8px] font-bold text-white/80 tracking-[0.2em] uppercase mb-1 block ml-2">
                  {txt.room}
                </label>
              ...
              {/* Error Message */}
              {loginError && (
                <p className="text-red-300 text-[10px] text-center font-bold bg-red-900/30 py-1 rounded">
                  {loginError}
                </p>
              )}
              ...
              <div className="pt-2 flex justify-center">
                <button
                  onClick={handleLogin}
                  className="w-full px-8 bg-white text-slate-900 py-2.5 rounded-full font-bold text-[10px] shadow-lg hover:bg-slate-100 hover:shadow-xl active:scale-95 transition-all uppercase tracking-[0.2em]"
                >
                  {txt.start}
                </button>
              </div>

              <p className="text-center text-[9px] text-white/50 mt-3 cursor-pointer hover:text-white hover:underline transition-all">
                {txt.help}
              </p>
            </div>
          </div>
          <div className="text-center mt-8">
            <p className="text-[8px] text-white/30 uppercase tracking-widest">
              © 2025 Swiss-Belhotel International
            </p>
          </div>
        </div>
      </div>
  ...
  // --- VIEW 2: MENU (FIXED PLUS ICON WITH SVG) ---
  if (view === 'menu') {
      ...
      <div className="min-h-screen bg-slate-50 font-sans pb-32">
        ...
        <div className="bg-white sticky top-0 z-30 px-6 pt-8 pb-4 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]">
          <div className="flex items-center justify-between mb-6">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`w-2 h-2 rounded-full animate-pulse ${hour >= 7 && hour <= 22 ? 'bg-green-500' : 'bg-red-500'}`}
                ></span>
                <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">
                  {hour >= 7 && hour <= 22 ? 'OPEN FOR SERVICE' : 'CLOSED'}
                </p>
              </div>
              <h2 className="text-2xl font-serif font-bold text-slate-900 leading-tight">
                {greeting} <br /> Room 1024
              </h2>
            </div>
          ...
          <div className="relative mb-6">
            ...
            <input
              ...
              className="w-full pl-12 pr-6 py-3.5 bg-slate-50 border border-slate-200 rounded-full text-sm focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition-all shadow-inner" />
          </div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-6 py-2.5 rounded-full text-xs font-bold whitespace-nowrap transition-all border ${selectedCategory === cat
        ...
        <div className="p-6 space-y-8">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-lg text-slate-900">Featured Menu</h3>
            <span className="text-xs text-orange-600 font-bold cursor-pointer">
              View All
            </span>
          </div>

          {MENU_ITEMS.filter((item) => item.category === selectedCategory).map(
            (item) => (
              <div
                key={item.id}
                className="bg-white rounded-[2rem] overflow-hidden shadow-xl shadow-slate-200/60 border border-slate-100 group"
              >
                <div className="relative h-64 w-full">
                  ...
                  <div className="absolute top-5 left-5 flex gap-2">
                    {item.tag && (
                      <span className="bg-white/95 backdrop-blur px-3 py-1 rounded-full text-[10px] font-bold text-slate-900 shadow-sm tracking-wide uppercase">
                        {item.tag}
                      </span>
                    )}
                  </div>
                ...
                <div className="px-6 pb-8 -mt-8 relative z-10">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      {/* --- TOMBOL ADD (PAKE SVG MANUAL BIAR MUNCUL) --- */}
                      <button
                        onClick={() => openItemDetail(item)}
                        className="w-10 h-10 bg-orange-600 rounded-full flex items-center justify-center text-white shadow-lg shadow-orange-500/40 hover:scale-110 active:scale-90 transition-transform"
                      >
                        {/* Kode SVG ini menggambar tanda + secara manual */}
                    ...
                    <div className="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                      <span className="w-3 h-3 rounded-full border border-slate-300 flex items-center justify-center text-[8px]">
                        i
                      </span>{' '}
                      20m
                    </div>
        ...
        {cart.length > 0 && (
          <div className="fixed bottom-8 left-6 right-6 z-40 animate-slide-up">
            <button
              ...
              className="w-full bg-orange-600 text-white p-1.5 pr-6 rounded-full shadow-2xl shadow-orange-500/50 flex justify-between items-center ring-4 ring-white"
            >
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-white text-orange-600 rounded-full flex items-center justify-center font-bold text-lg shadow-inner">
                  {cart.length}
                </div>
                <div className="text-left">
                  <p className="text-[9px] text-orange-100 uppercase tracking-widest font-bold mb-0.5">
                    {txt.total}
                  </p>
              ...
              <div className="flex items-center gap-2 font-bold text-xs tracking-widest uppercase">
                {txt.cart} <ChevronRight className="w-4 h-4" />
              </div>
            </button>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-end animate-fade-in">
            <div className="bg-white w-full rounded-t-[2.5rem] p-8 max-h-[90vh] overflow-y-auto animate-slide-up shadow-2xl">
              ...
              <div className="mb-8">
                ...
                <textarea
                  className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm focus:outline-none focus:border-orange-500 transition-all"
                  rows={3}
              ...
              <div className="flex gap-4 items-center">
                <div className="flex items-center bg-slate-100 rounded-full p-1">
                  <button
                    ...
                    className="w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center text-slate-900 font-bold hover:bg-slate-50"
                  >
                ...
                <button
                  ...
                  className="flex-1 bg-slate-900 text-white py-4 rounded-full font-bold text-sm shadow-xl hover:bg-slate-800 active:scale-95 transition-all"
                >
                  Add to Cart - Rp{' '}
        ...
        {/* --- CART MODAL (UPDATED WITH CHECKOUT BUTTON) --- */}
        {showCartModal && (
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-end animate-fade-in">
            <div className="bg-white w-full rounded-t-[2.5rem] p-8 max-h-[85vh] overflow-y-auto animate-slide-up shadow-2xl">
              <div className="flex justify-between items-center mb-8">
                ...
                <button
                  ...
                  className="p-2 bg-slate-50 rounded-full hover:bg-slate-100 transition-all"
                >
              ...
              <div className="space-y-6 mb-8">
                {cart.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex justify-between items-start border-b border-slate-50 pb-4 last:border-0"
                  >
                    <div className="flex items-start gap-4">
                      <div className="w-16 h-16 rounded-2xl bg-slate-100 overflow-hidden shadow-sm">
                        <img
                          src={item.image}
                          className="w-full h-full object-cover" />
                      </div>
                      <div>
                        ...
                        <p className="text-xs font-bold text-orange-600">
                          Rp {(item.price * item.qty).toLocaleString()}</p>
                        {item.note && (
                          <p className="text-[10px] text-slate-400 mt-1 italic bg-slate-50 px-2 py-1 rounded-md inline-block">
                            " {item.note} "
                          </p>
                        )}
                      </div>
                    ...
                    <button
                      ...
                      className="text-red-500 bg-red-50 px-3 py-1.5 rounded-full text-[10px] font-bold hover:bg-red-100 transition-colors"
                    >
                      Remove
                    </button>
                  </div>
              ...
              <button
                ...
                className="w-full bg-orange-600 text-white py-4 rounded-2xl font-bold text-sm shadow-xl shadow-orange-500/30 hover:bg-orange-700 active:scale-95 transition-all uppercase tracking-widest flex justify-center items-center gap-2"
              >
                {txt.checkout}
              </button>
            </div>
      ...
      <div className="min-h-screen bg-slate-50 font-sans pb-32">
        ...
        <div className="p-6 space-y-6">
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
            ...
            <div className="space-y-3">
              <label
                className={`flex items-center gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ${paymentMethod === 'room'
              ...
              >
                ...
                <div
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'room'
                      ? 'border-orange-500'
                      : 'border-slate-300'}`}
                >
                  {paymentMethod === 'room' && (
              ...
              <label
                className={`flex items-center gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ${paymentMethod === 'qris'
                    ? 'border-orange-500 bg-orange-50'
                    : 'border-slate-100'}`}
              >
                <input
                ...
                <div
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'qris'
                      ? 'border-orange-500'
                      : 'border-slate-300'}`}
                >
                  {paymentMethod === 'qris' && (
              ...
              <label
                className={`flex items-center gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ${paymentMethod === 'bank'
                    ? 'border-orange-500 bg-orange-50'
                    : 'border-slate-100'}`}
              >
                <input
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between text-slate-500">
                <span>{txt.tax}</span>
                <span>Rp {taxService.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-lg font-bold text-slate-900 mt-4 pt-4 border-t border-slate-100">
                <span>{txt.total}</span>
                <span>Rp {grandTotal.toLocaleString()}</span>
              </div>
            </div>
        ...
        <div className="fixed bottom-8 left-6 right-6 z-40">
          <button
            onClick={handleOrder}
            disabled={loading}
            className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold text-sm shadow-xl active:scale-95 transition-all flex justify-center items-center gap-2"
          >
            {loading ? (
    ...
    return (
      <div className="min-h-screen bg-green-50 flex flex-col items-center justify-center p-8 text-center font-sans">
        ...
        <button
          ...
          className="px-10 py-4 bg-white border border-slate-200 rounded-full text-sm font-bold text-slate-700 hover:bg-slate-50 transition-all shadow-sm uppercase tracking-widest"
        >
          {txt.back}
        </button>
      </div>
