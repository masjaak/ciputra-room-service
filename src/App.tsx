...
import { ShoppingCart, Plus, Minus, ChevronRight, Search, Star, MapPin, Phone, Globe, X, Info, Building2, QrCode, CreditCard, ChevronLeft } from 'lucide-react';
...
export default function App() {
  ...
  // --- VIEW 1: LOGIN (FULL SCREEN FIX) ---
  if (view === 'login') {
    return (
      // PERUBAHAN: 'fixed inset-0' memaksa element menuhin layar 100% tanpa sisa
      <div className="fixed inset-0 w-full h-full flex flex-col justify-center items-center p-6 font-sans overflow-hidden bg-black">
        ...
        <div className="absolute inset-0 z-0">
          {/* JANGAN LUPA GANTI LINK BACKGROUND DI SINI */}
        ...
        <div className="relative z-10 w-full max-w-[280px] mx-auto flex flex-col h-full justify-center">
          ...
          <div className="text-center mb-8 animate-fade-in-up">
             ...
             {/* 🔥🔥🔥 GANTI LINK LOGO DI BAWAH INI YA MAS! 🔥🔥🔥 */}
             <img 
             ...
             <p className="text-white/90 font-light italic text-[10px] tracking-widest uppercase border-t border-white/30 inline-block pt-2 px-4">
          ...
          <div className="bg-white/10 backdrop-blur-xl border border-white/20 px-5 py-6 rounded-[1.5rem] shadow-2xl ring-1 ring-white/30 w-full">
            <div className="space-y-3"> 
              ...
              {loginError && (
                <p className="text-red-300 text-[10px] text-center font-bold bg-red-900/30 py-1 rounded animate-pulse">{loginError}</p>
              )}

              <div className="pt-2 flex justify-center">
                <button 
                  onClick={handleLogin} 
                  className={`w-auto px-8 py-2.5 rounded-full font-bold text-[10px] shadow-lg active:scale-95 transition-all uppercase tracking-[0.2em] ${!roomNumber || !phoneNumber ? 'bg-slate-400 text-slate-200 cursor-not-allowed' : 'bg-white text-slate-900 hover:bg-slate-100 hover:shadow-xl'}`}
                >
                  {txt.start}
                </button>
              </div>
        ...
        {selectedItem && (
          <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-end animate-fade-in">
            <div className="bg-white w-full rounded-t-[2.5rem] p-8 max-h-[90vh] overflow-y-auto animate-slide-up shadow-2xl">
               ...
               <div className="mb-8"><label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">{txt.addNote}</label><textarea className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-sm focus:outline-none focus:border-orange-500 transition-all" rows={3} placeholder="E.g. No onions, extra spicy..." value={itemNote} onChange={(e) => setItemNote(e.target.value)}></textarea></div>
